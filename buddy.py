“””
buddy.py — A fully offline AI companion. Pure Python 3 stdlib only.
Designed to be genuinely useful as a friend to talk to when bored.
Capabilities: small talk, jokes, advice, venting support, trivia, deep convos.
“””
...
NAME = “Buddy”
MOOD_RESPONSES = {
“happy”:   [“That’s awesome, I’m glad!”, “Love to hear it!”, “You’re radiating good vibes right now.”],
“sad”:     [“Hey, I’m here. Want to talk about it?”, “That sounds rough. I’ve got time.”, “I’m listening. No rush.”],
“bored”:   [“Okay let’s fix that. Ask me anything — trivia, deep questions, dumb jokes.”, “Boredom is just curiosity with nowhere to go yet. Let’s find somewhere.”, “Tell me the weirdest thing on your mind right now.”],
“angry”:   [“Oof. What happened?”, “Let it out. I won’t judge.”, “That sounds genuinely frustrating. Tell me more.”],
“anxious”: [“Take a breath. I’m right here.”, “Anxiety lies a lot. What’s the actual worst case?”, “Let’s think through it together, no pressure.”],
“tired”:   [“Rest when you can. But I’m here if you want to talk.”, “Tired body or tired mind?”, “Even exhausted people deserve a good conversation.”],
}
...
(“I asked the library if they had books on paranoia.”, “The librarian whispered: they’re right behind you.”),
...
# MARKOV CHAIN (fallback generation)
...
def respond(self, text):
    ...
    if re.search(r'(what\'?s your name|who are you|what are you|tell me about yourself)', tl):
        return ("I'm Buddy — a fully offline AI companion. No internet, no cloud, no data collection. "
                "Just Python and a lot of pre-loaded personality. I'm here to talk whenever you're bored.")
    ...
    if re.search(r'\b(advice|help me|what do i do|should i|guide me)\b', tl):
        topic = "life"
        if re.search(r'\b(friend|friendship|relationship|people)\b', tl): topic = "friendship"
        elif re.search(r'\b(motivat|lazy|procrastinat|stuck|start)\b', tl): topic = "motivation"
        elif re.search(r'\b(stress|overwhelm|anxiety|anxious|pressure)\b', tl): topic = "stress"
        elif re.search(r'\b(bored|boring|nothing|hobby)\b', tl): topic = "boredom"
        return random.choice(ADVICE[topic])

    # ── Vent / talk about feelings ──
    ...
    # ── Mood detection standalone ──
    mood = self._detect_mood(tl)
    if mood:
        return random.choice(MOOD_RESPONSES[mood])

    # ── What can you do ──
    if re.search(r'\b(what can you do|capabilities|help with|can you|features)\b', tl):
        return ("I can: tell jokes, share wild trivia, give real advice, chat about feelings, "
                "ask you deep questions, suggest things to do when bored, or just vibe. "
    ...
    # ── Fallback: Markov + reflective nudge ──
    reflective = [
...
if line == ‘**EXIT**’: break
print(buddy.respond(line), flush=True)
